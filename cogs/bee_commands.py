...
class BeeCommands(vbu.Cog):
    ...
    @bee.command(name="testbreed")
    @vbu.defer()
    @commands.guild_only()
    async def bee_testbreed(self, ctx: vbu.Context):
        """
        Breed one of your princesses and drones into a queen.
        """
        ...
        await payload.defer_update()  # I don't think this is necessary but we'll keep it anyway
        return await payload.message.edit(
            content=f"These two would make a {utils.BeeType.combine(princess.type, drone.type)} bee.",
            components=None,
            allowed_mentions=discord.AllowedMentions.none(),
            wait=False,
        )
